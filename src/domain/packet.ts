...
import { formatCompact, type Currency } from "./money";
...
export interface PacketOptions {
  ...
  /** What the value is denominated in. Defaults to USD for existing callers. */
  currency?: Currency;
}
...
export function buildPacket(card: PacketCard, options: PacketOptions): string {
  ...
  return [
    ...
    `Value (working): ${card.value ? formatCompact(card.value, options.currency ?? "USD") : "—"} · ${card.tier ?? "core"}${card.whale ? " · WHALE" : ""}`,
